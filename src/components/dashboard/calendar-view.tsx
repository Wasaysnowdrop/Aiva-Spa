    ...
    <div className="flex flex-col gap-5 p-5">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-[#8A8F98]">Upcoming bookings captured by the chat widget. Enabled reminders are queued automatically by email and SMS.</p>
...
function EditorDialog({ open, booking, leads, prefillLeadId, timezone, anchor, onClose, onSaved }: { open: boolean; booking: CalendarBooking | null; leads: Lead[]; prefillLeadId: string | null; timezone: string; anchor: string; onClose: () => void; onSaved: (booking: CalendarBooking) => void }) {
  ...
  const [smsReminder, setSmsReminder] = React.useState(booking?.reminderSmsEnabled ?? true)
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    setSaving(true)
    try {
      const result = await saveBookingAction({ id: booking?.id, leadId: leadId || null, startAtIso, durationMinutes: Number(duration), service: service.trim(), visitorName: name.trim(), visitorEmail: email.trim(), visitorPhone: phone.trim(), timezone, notes, reminderEmailEnabled: emailReminder, reminderSmsEnabled: smsReminder })
